...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < (((unsigned int)argc % 2 == 0) ? (unsigned int)(argc - 1)
                                                 : (unsigned int)argc);
       ++i) {
    if (i % 2 == 1) {
      if ((strlen(argv[i]) > 2) && strstr(argv[i], "-")) {
        ...
        if (strcmp(large_option, "di") == 0) {
          ...
          if (strcmp(current_arg, "true") == 0) {
            ...
            printf(
                "'-entire' has a wrong value '%s'. It must be a boolean value, "
                "'true' or 'false'\n",
                current_arg);
            return close_dataset_and_exit(dataset);
          }
        ...
        case 't':
          ...
          if ((*arg_end) || (threshold <= 0)) {
            printf(
                "'-t' has a wrong value '%s'. It must be a float greater than "
                "0.0\n",
                current_arg);
        ...
        case 'p':
          ...
          if ((*arg_end) || (promotion <= 0)) {
            printf(
                "'-p' has a wrong value '%s'. It must be a float greater than "
                "0.0\n",
                current_arg);
        ...
        case 'd':
          ...
          if ((*arg_end) || (demotion <= 0)) {
            printf(
                "'-d' has a wrong value '%s'. It must be a float greater than "
                "0.0\n",
                current_arg);
            return close_dataset_and_exit(dataset);
          }
    ...
    if (isdigit(argv[argc - 1][0])) {
      ...
      do {
        ...
        test_directory = (char *)realloc(
            test_directory,
            (snprintf(NULL, 0, TEST_DIRECTORY, time_nanoseconds, given_number) +
             1) *
                sizeof(char));
      ...
      fprintf(file, "f=%s\nt=%f\np=%f\nd=%f\nb=%u\ne=%zu\nr=%u\nrun=%d\n",
              dataset_value, threshold, promotion, demotion, breadth,
              iterations, max_rules_per_instance, given_number);
  ...
  if (!entire) {
    ...
    if (train_test_split(dataset, has_header, testing_ratio, &split_rng,
                         train_path, NULL, NULL, NULL) != 0) {
  ...
  if (!partial_observation) {
    if (labels) {
      Sensor *sensor = sensor_constructor_from_file(train_path, delimiter,
                                                    false, has_header);
      ...
      for (i = 0; i < total_observations; ++i) {
        ...
        if (result->size == 0) {
          char *str = scene_to_string(observation),
               *labels_str = scene_to_string(labels);
          printf("labels files does not contain a label for "
                 "observation\nLabels: %s\nObservation:"
                 " %s\n",
                 labels_str, str);
      ...
      if (test_directory) {
        ...
        nerd_at_instance_filename = (char *)calloc(
            (snprintf(NULL, 0, "%siteration_%zu-instance_%zu.nd",
                      test_directory, iterations, total_instances) +
             1),
            sizeof(char));

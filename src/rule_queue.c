 ...
 *
 * @param take_ownership Indicates whether the RuleQueue should take onwership
 * of the Rules that will be added or just keep their reference. If true is
 ...
 *
 * @param destination The RuleQueue to save the copy. It should be a reference
 * to the struct's pointer (to a RuleQueue *).
 * @param source The RuleQueue to be copied. If the RuleQueue is NULL, the
 * contents of the destination will not be changed.
 */
...
/**
 * @brief Shows whether the RuleQueue is taking the ownership of the Rules that
 ...
 * @param rule The Rule to be enqueued. It should be reference to a Rule * (Rule
 * ** - a pointer to a Rule *). If the given rule_queue was constructed to take
 * ownership, this parameter will become NULL. If not, it will not become NULL.
 * If NULL is given, the queue will remain the same.
 */
 ...
 * @param dequeued_rule A place to save the Rule that will be dequeued. It
 * should be a reference to the struct's pointer (Rule **). Make sure that the
 * given double pointer is not already allocated, otherwise its contents will be
 * lost in the memory. If NULL is given, the Rule will be destroyed.
 */
  ...
  if (rule_queue) {
    if (rule_queue->rules) {
      ...
      if (rule_queue->length == 0) {
        ...
        rule_queue->rules =
            (Rule **)malloc(rule_queue->length * sizeof(Rule *));

        memcpy(rule_queue->rules, (rules + 1),
               rule_queue->length * sizeof(Rule *));
 ...
 *
 * @return index where the Rule is or -1 if it does not exist or either the Rule
 * or the RuleQueue are NULL.
 */
 ...
 * @param removed_rule A place to save the Rule that will be removed. It should
 * be a reference to the struct's pointer (Rule **). Make sure that the given
 * double pointer is not already allocated, otherwise its contents will be lost
 * in the memory. If NULL is given, the Rule will be destroyed.
 */
...
/**
 * @brief Find all the applicable Rules with a given Context. An applicable
 * Rule, is a Rule whose body is true.
 *
 * @param rule_queue The RuleQueue to find the applicable Rules from.
 * @param context The Context to check for applicable Rules.
 * @param rule_indices The IntVector to save the indices of the Rules that are
 * applicable. It should be a reference to the struct's pointer (to a IntVector
 * *).
 */
...
/**
 * @brief Finds all the concurring rules with a given Context. A concurring
 * Rule, is a Rule whose body and its head are true.
 *
 * @param rule_queue The RuleQueue to find the concurring Rules from.
 * @param context The Context to check for concurring Rules.
 * @param rule_indices The IntVector to save the indices of the Rules that are
 * concurring. It should be a reference to the struct's pointer (IntVector *).
 */
 ...
 *
 * @return The string format of the given RuleQueue. Use free() to deallocate
 * this string. Returns NULL if the RuleQueue is NULL.
 */

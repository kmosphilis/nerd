 ...
 *
 * @param context The context to be destructed. It should be a reference to the
 * struct's pointer (to a Context *).
 */
void context_destructor(Context **const context) { scene_destructor(context); }

/**
 * @brief Adds a Literal to the Context by taking its ownership. Alias for a
 * scene_add_literal.
 *
 * @param context The Context to be expanded.
 * @param literal_to_add The Literal to add in the Context. It should be a
 * reference to a Literal * (Literal ** - a pointer to a Literal *). Upon
 * succession, this parameter will become NULL.
 */
 ...
 *
 * @return The Prudens JS Context format (as a string) of the given Context. Use
 * free() to deallocate the result. Returns NULL if the Context or its body are
 * NULL.
 */

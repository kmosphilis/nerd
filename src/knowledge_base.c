 ...
 *
 * @param activation_threshold The threshold which determines whether a rule
 * should get activated or deactivated.
 ...
 *
 * @param knowledge_base The KnowledgeBase to be destructed. It should be a
 * reference to the object's pointer. If NULL the process will fail.
 */
 ...
 * @param source The KnowledgeBase to be copied. If the KnowledgeBase is NULL,
 * the contents of the destination will not be changed.
 */
...
/**
 * @brief Adds a Rule in the KnowledgeBase by taking its ownership. If the
 * weight of the Rule is above the activation_threshold, the Rule will be added
 * to the active RuleQueue, otherwise it will be added to the inactive
 * RuleQueue.
 *
 * @param knowledge_base The KnowledgeBase to be expanded.
 * @param rule The Rule to be added. It should be reference to a Rule * (Rule **
 * - a pointer to a Rule *). If the given Rule had took ownership of its
 * content, a new Rule will be created without taking the ownership of that
 * content as it will belong to the internal RB-Tree. If it does not take the
 * ownership of the its content, the content pointers might stil change, as the
 * body and head will take the pointer of the item allocated in the RB-Tree.
 *
 * @return 1 if the Rule was added successfully, 0 if it was not added, and -1
...
/**
 * @brief Creates new Rules by finding uncovered Literals. Uncovered Literals,
 * are Literals that have been observed, but have not been inferred.
 *
 * @param knowledge_base The KnowledgeBase to be expanded.
 ...
 * @return The string format of the given KnowledgeBase. Use free() to
 * deallocate this string. Returns NULL if the KnowledgeBase is NULL.
 */
char *knowledge_base_to_string(const KnowledgeBase *const knowledge_base) {
  if (knowledge_base && knowledge_base->active && knowledge_base->hypergraph) {
    ...
    const char *const beginning = "Knowledge Base:\n",
                      ...
                      *const inactive_string = "Inactive Rules: ",
                      *const threshold = "Activation Threshold: ";

    size_t result_size = strlen(beginning) + strlen(active_string) +
                         strlen(inactive_string) + strlen(threshold) + 1;

    char weight_length[50];
    int weight_size =
        sprintf(weight_length, "%.4f", knowledge_base->activation_threshold);
    ...
    sprintf(result, "%s%s%.4f\n%s%s\n", temp, threshold,
            knowledge_base->activation_threshold, active_string,
            rule_queue_string);
...
/**
 * @brief Converts a KnowledgeBase to a Prudens JS Knowledge base format. Note:
 * It only returns the active rules of a KnowledgeBase, since the inactive
 * should not be considered until their activation.
 *
 * @param knowledge_base The KnowledgeBase to be converted.
 * @return The Prudens JS Knowledge base format (as a string) of the given
 * KnowledgeBase. The rules will be in reverse order since younger rules in
 * Prudens JS have higher priority unlike NERD, where older rules have higher
 * priority. Use free() to deallocate the result. Returns NULL if the given

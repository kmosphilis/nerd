 ...
 *
 * @return A newly allocated trimmed string, or NULL if string is whitespaces
 * only. Use free to deallocate.
 */
 ...
 * @param has_header Boolean value which indicates whether the given dataset has
 * boolean values or not.
 * @param test_ratio A float indicating the ratio of the testing dataset, given
 * the dataset's size.
 * @param generator A pcg32_random_t pointer to an RNG.
 * @param train_path A string containing the filename location to save the
 * training dataset. If NULL, the train will contain a tmpfile.
 * @param test_path A string containing the filename location to save the
 * testing dataset. If NULL, the test parameter will contain a tmpfile.
 * @param train A double pointer - reference to a FILE * - &(FILE *) - to save
 * the training dataset. If NULL, the training split will not be saved.
 * @param test A double pointer - reference to a FILE * - &(FILE *) - to save
 * the testing dataset. If NULL, the testing split will not be saved.
 *
 * @return 0 if the process was successful, -1 if the given train_path was
 * incorrect and the train dataset was saved in a tmpfile, -2 if the given
 * test_path was incorrect and the test dataset was saved in a tmpfile, or -3 if
 * both were incorrect.
 */
int train_test_split(FILE *dataset, const bool has_header,
  ...
  unsigned int *possible_indices =
      (unsigned int *)malloc(dataset_size * sizeof(int));
 ...
 *
 * @param int_vector The IntVector to be destructed. If NULL the process will
 * fail. It should be a reference to the object's pointer.
 */
 ...
 * @param source The IntVector to be copied. If NULL is given, the contents of
 * the destination will not be changed.
 */
...
/**
 * @brief Resizes an IntVector. If new_size > int_vector.size, the new elements
 * will be set to 0;
 *
 ...
 *
 * @param int_vector The IntVector to push the new element in. If NULL the
 * process will fail.
...
/**
 * @brief Expands the IntVector by adding a new item at the given position. The
 * item will be added to that position, and all subsequent values will have an
 * index = old index + 1. If the given position is equal to the size of the
 * vector, the element will function like int_vector_push().
 *
 * @param int_vector The IntVector to add the new element in. If NULL the
 * process will fail.
 ...
 *
 * @param int_vector The IntVector to delete the item from. If NULL the process
 * will fail.
 * @param index The index of the item to be removed. If index > int_vector.size,
...
/**
 * @brief Retrieves the item from the given index position. Equivalent to
 * int_vector.items[index] but with checks.
 *
 * @param int_vector The IntVector to retrieve the item from. If NULL the
 * process will fail.
 * @param index The index of the required item. If index > int_vector.size, the
 * process will fail.
 *
 * @return The item (int) at the given index, or the index if out of bounds or
 * int_vector is NULL.
 */
int int_vector_get(const IntVector *const int_vector,
...
/**
 * @brief Sets the given item (int) to the given index. Equivalent to
 * int_vector.items[index] = new_item but with checks.
 *
 * @param int_vector The IntVector to set the new item to. If NULL the process
 * will fail.
 * @param index The index of the item to be replaces. If index >
 * int_vector.size, the process will fail.

...
#define _ck_assert_knowledge_base_empty(X, OP, COMP)                           \
  do {                                                                         \
    ...
    ck_assert_msg(((_kb->active->length OP 0)COMP(result_size OP 0)),          \
                  "Assertion 'knowledge_base%sempty' failed: number of "       \
                  "active rules = %zd, "                                       \
                  "number of inactive rules = %zd",                            \
                  " " #OP " ", _kb->active->length, result_size);              \
  } while (0)
 ...
 */
#define ck_assert_knowledge_base_empty(X)                                      \
  _ck_assert_knowledge_base_empty(X, ==, &&)
 ...
 * @param knowledge_base The KnowledgeBase to check.
 */
#define ck_assert_knowledge_base_notempty(X)                                   \
  _ck_assert_knowledge_base_empty(X, !=, ||)

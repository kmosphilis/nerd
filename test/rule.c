...
START_TEST(construct_destruct_test) {
  Literal *body[BODY_SIZE], *body_copy[BODY_SIZE],
      *head = literal_constructor("Fly", 0), *head_copy;
  ...
  for (i = 0; i < BODY_SIZE; ++i) {
    ...
    body_copy[i] =
        literal_constructor(body_literal_atoms[i], body_literal_signs[i]);
  }
  ...
  rule = rule_constructor(BODY_SIZE, body_copy, &head_copy, starting_weight,
                          false);
...
START_TEST(copy_test) {
  ...
  Rule *rule1 = rule_constructor(BODY_SIZE, body, &head, starting_weight, true),
       *rule2;
  ...
  rule1 = rule_constructor(BODY_SIZE, body_copy, &head_copy, starting_weight,
                           false);
  rule_copy(&rule2, rule1);
...
START_TEST(equality_checK_test) {
  Rule *rule1 = NULL, *rule2 = NULL, *rule3 = NULL, *rule4 = NULL,
       *rule5 = NULL, *rule6 = NULL, *rule7 = NULL;
  Literal *body[BODY_SIZE], *head = literal_constructor("Fly", 0), *head_copy,
                            *body_copy[BODY_SIZE];
  literal_copy(&head_copy, head);
  char *body_literal_atoms[BODY_SIZE] = {"Penguin", "Bird", "Antarctica"};
  ...
  rule2 = rule_constructor(BODY_SIZE, body_copy, &head,
                           starting_weight - 0.00001, true);
  ...
  rule5 = rule_constructor(BODY_SIZE - 1, body_copy, &head_copy,
                           starting_weight, true);
  ...
  for (i = 0; i < BODY_SIZE; ++i) {
    body[i] =
        literal_constructor(body_literal_atoms2[i], body_literal_signs[i]);
  }
...
START_TEST(to_string_test) {
  ...
  rule->weight = 3.1415;
  rule_string = rule_to_string(rule);
  ck_assert_str_eq(rule_string,
                   "(penguin, bird, antarctica) => wings (3.1415)");
  free(rule_string);
...
START_TEST(to_prudensjs_test) {
  ...
  ck_assert_str_eq(rule_prudensjs_string,
                   "{\"name\": \"Rule1\", \"body\": ["
                   "{\"name\": \"penguin\", \"sign\": true, \"isJS\": false, "
                   "\"isEquality\": false, "
                   "\"isInEquality\": false, \"isAction\": false, \"arity\": "
                   "0}, {\"name\": \"bird\", "
                   "\"sign\": true, \"isJS\": false, \"isEquality\": false, "
                   "\"isInEquality\": false, "
                   "\"isAction\": false, \"arity\": 0}, {\"name\": "
                   "\"antarctica\", \"sign\": true, "
                   "\"isJS\": false, \"isEquality\": false, \"isInEquality\": "
                   "false, \"isAction\": false, "
                   "\"arity\": 0}], \"head\": {\"name\": \"fly\", \"sign\": "
                   "false, \"isJS\": false, "
                   "\"isEquality\": false, \"isInEquality\": false, "
...
Suite *rule_suite() {
  Suite *suite;
  TCase *create_case, *copy_case, *weight_adjustment_case, *equality_check_case,
      *convert_case;
